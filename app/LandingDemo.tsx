...
export default function LandingDemo({ diagrams }: { diagrams: Demo[] }) {
  const demos = diagrams.slice(0, 6);
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1120, margin: "0 auto", padding: "0 24px 80px" }}>
        ...
        {/* Gallery */}
        {demos.length > 0 && (
          <section className="ld-grid ld-in" style={{ display: "grid", gap: 18 }}>
            {demos.map((d, i) => (
              <a key={d.id} href={`/d/${d.id}`} className="ld-card" style={{
                ...
                <div style={{ height: 190, background: "#fbfbfd", display: "flex", alignItems: "center", justifyContent: "center", padding: 14, overflow: "hidden" }}>
      ...
      <style>{`
        ...
        @media (max-width: 560px) { .ld-grid { grid-template-columns: 1fr; } h1 { font-size: 30px !important; } }
