...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("[diagrams] GET error:", err instanceof Error ? err.message : String(err));
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("[diagrams] POST error:", err instanceof Error ? err.message : String(err));
    return NextResponse.json({ error: "Internal error" }, { status: 500 });
  }
}
